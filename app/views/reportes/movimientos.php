<?php
// Fragmento de contenido: se renderiza dentro de layouts/main.php (ver ReportesController::movimientos()).
// Solo presentación: consultas, filtros backend, exportaciones y lógica de fechas NO cambian. Los
// nombres/IDs de los campos de filtro y los parámetros GET (fecha_desde, fecha_hasta, tipo,
// id_responsable, formato) se conservan EXACTAMENTE. Flatpickr local intacto.
...
<div class="page-header">
    <div class="page-header-fila">
        <div>
            <h1 class="page-title">Movimientos por período</h1>
        </div>
...
<form method="GET" action="index.php" class="filters">
    ...
    <div class="form-group">
        <label class="form-label" for="id_responsable">Responsable relacionado</label>
        <select id="id_responsable" name="id_responsable" class="form-control">
            <option value="">Todos</option>
            <?php foreach ($responsables as $responsable): ?>
                <option value="<?= (int) $responsable['id_responsable'] ?>" <?= ((int) ($filtros['id_responsable'] ?? 0) === (int) $responsable['id_responsable']) ? 'selected' : '' ?>><?= htmlspecialchars($responsable['nombre_completo'], ENT_QUOTES, 'UTF-8') ?></option>
            <?php endforeach; ?>
        </select>
    </div>
    ...
    <div class="form-actions-inline">
        ...
        <a href="index.php?modulo=reportes&accion=movimientos" class="btn btn-secondary">Limpiar filtros</a>
    </div>
</form>

<div class="card">
    <?php if ($error === null && empty($filas)): ?>
        <p class="estado-vacio">No se encontraron movimientos para los filtros seleccionados.</p>
    <?php elseif ($error === null): ?>
        <div class="table-responsive">
            <table class="table-app table-detail-centered table-resizable table-reporte-movimientos">
                <thead>
                    <tr>
                        <th>Fecha</th>
                        <th>Tipo</th>
